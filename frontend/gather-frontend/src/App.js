...
export default function App() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("https://api.gatherapp.live/recommendations", {
        method: "POST",
            ...
            <Field label="Where are you?">
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button onClick={handleUseMyLocation} style={{
                  background: coords ? "rgba(74,222,128,0.15)" : "rgba(255,255,255,0.04)",
                  border: `1px solid ${coords ? "#4ade80" : "rgba(255,255,255,0.1)"}`,
                  borderRadius: 8,
                  padding: "12px 16px",
                  color: coords ? "#4ade80" : "#6b7280",
                  fontFamily: "monospace",
                  fontSize: 13,
                  cursor: "pointer",
                  whiteSpace: "nowrap",
